...
let FlashSaleMiddleware = (req, res, next) => {
    ...
    jwt.verify(token, config.parsed.ACCESS_TOKEN_SECRET, async (err, user) => {
        ...
        if (user.user_flashsale != true) {
            res.status(300)
                .json({
                    status: 'failed',
                    message: 'not a flashsale participant',
                    data: null,
                    error: 'not a flashsale participant'
                })
...
module.exports = FlashSaleMiddleware;